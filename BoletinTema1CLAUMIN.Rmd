...
date:  "`14/02/2023`"  #Pondría la fecha del día actual
params:
  lang: ES
...
#Primero estudiamos el conjunto de datos mammals
dim(mam)
#la dimensión es 62 filas por dos columnas
head(mam)
tail(mam)
str(mam)

#Segundo estudiamos el conjunto de datos Animals2

ani <-B

dim(ani)
#tiene una dimension de unas 65 filas y 2 columnas
head(ani)
tail(ani)
str(ani)
...
ani.no <- setdiff(rownames(ani),rownames(mam))
ani.no

#son 7 Son en su mayoría dinosaurios

ani.no2 <- setdiff(rownames(mam),rownames(ani))
ani.no2
#son 4 y son mamíferos
...
#"Artic ground squirrel""Arctic ground squirrel"
#hay algunos que estan mal escritos y hacen la diferencia entre ambos conjuntos.
...
A_1 <- MASS::mammals
A_2 <- robustbase::Animals2
#Primero lo hacemos con los mamíferos
b.w <- A_1$brain


car::qqPlot(log(b.w))
#Ahora con el conjunto de Animals2

b.w.a2 <- A_2$brain

car::qqPlot(log(b.w.a2))

#No presentan la misma distribución ya que la primera tiene una pendiente más pronunciada

#Creo que ambas siguen una distribución Gaussiana
```
...
# "abind"  "askpass"     "assertthat" "backports"   "base64enc" 
...
#hay 1 na en age 11 en education 98 en income 17 en statusquo y 168 en vote
...
library(MASS)
MASS::cabbages
#determinamos los diferentes tipos de calabaza que hay
levels(cabbages$Cult)

#representamos ambos
hist(cabbages$VitC[cabbages$Cult=="c39"],col="red")
hist(cabbages$VitC[cabbages$Cult=="c52"],col="cyan",add=TRUE)   
...
#representación 
library(MASS)
plot(cabbages$VitC,cabbages$HeadWt,pch = c(6,16)[cabbages$HeadWt])
legend(x="topright",legend=c("VitC","HeadWt"), pch=c(6,16))

#creamos un modelo
Model3 <- lm(VitC ~ HeadWt, data = cabbages)
abline(Model3,lty=4,col="red")
#b)
#representación de modelo de la calabaza 

plot(cabbages$HeadWt,cabbages$Cult,pch =c(2,4)[cabbages$Cult])
plot(cabbages$VitC,cabbages$Cult,pch =c(6,16)[cabbages$Cult])

Model4 <- lm(Cult ~VitC ,data=cabbages,subset=which(Cult=="c39"))
Model6 <- lm(Cult ~VitC ,data=cabbages,subset=which(Cult=="c52"))
abline(Model4,lty=3)
abline(Model6,lty=3)


Model5 <- lm(Cult ~HeadWt ,data=cabbages,subset=which(Cult=="c39"))
Model7 <- lm(Cult ~HeadWt ,data=cabbages,subset=which(Cult=="c52"))
